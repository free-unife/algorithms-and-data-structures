...
double
runningTime (clock_t start, clock_t end)
{
  return (((double) (end - start) / (double) CLOCKS_PER_SEC));
}

void
initArray (int **arrayPtr)
{
...
arraySum (int *array)
{
  return (arrayEmpty (array) ? 0 : el (array, 0) + arraySum (pt (array, 1)));
}

bool
arrayEqual (int *a1, int *a2)
{
  /*
   * Base case. 
   */
...
copyArray (int *src)
{
   ...
   */
  return (memcpy (dest, src, (arrayLength (src) + 1) * sizeof (int)));
}

int *
genRandomArray (int len)
{
  int i;
  int *array;
  struct timespec t1;
  ...
  for (i = 0; i < len; i++)
    {
      ...
      assert (ge (el (array, i), MIN_RAND) && le (el (array, i), MAX_RAND));
    }
